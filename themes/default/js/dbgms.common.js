var dbgms_common_url;
/* 刷新验证码 */
function dbgms_captcha(id, url, sign) {
	document.getElementById(id).src = url + 'user/checkyzm/' + sign + '?' + Math.random();
	// document.getElementById(id).src = '/passport/checkyzm?' + Math.random();
}
/* dbgms通用js：ajax提交 */
(function(dbgms) {
	dbgms.fn.captcha = function(config) {
		var default_config = {
			"float" : "left",
			minStatue : !1,
			skin : "red",
			durationTime : 1e3,
			isopen : false
		}, config = dbgms.extend(default_config, config);
...
/* dbgms通用js：ajax提交 */
(function(dbgms) {
	dbgms.fn.submit = function(config) {
		var default_config = {
			"float" : "left",
			minStatue : !1,
			skin : "red",
...
/* 内容更新或者提交 */
function dbgms_formsubmit(form, ajaxurl, jumpurl) {
	form = form || '';
	if (form == '') {
		alert('表单不能为空!');
		return;
	...
	form.find(" input[type=text],textarea").each(function() {
		if ($(this).val() == "") {
			...
			if (isnull == 0) {
				...
				ispass = true;
				/*
				 * if (action == 'register') { if ($('#regpwd1').val() !=
				 * $('#regpwd2').val()) { alert('两次密码不一致!'); return false; } }
				 */
			}
		}
	});
	...
	// map
	/*
	 * var str_data = $("#"+formname+" input").map(function() { return
	 * ($(this).attr("name") + '=' + $(this).val()); }).get().join('&');
	 */
	if (ispass == true) {
		$.ajax({
			url : ajaxurl,
...
/*
 * 焦点 $(function(){$('#username').focus();});
 */
/* 内容更新或者提交 */
function dbgjs_submit(form, ajaxurl, jumpurl) {
	form = form || '';
	ajaxurl = ajaxurl || '';
	jumpurl = jumpurl || ajaxurl;
	var ispass = false, isnull = 0;
...
function dbgjs_login() {
	if ($('#name').val() == '') {
		...
		alert('请输入密码');
		$('#password').focus();
	} else if ($('#checkyzm').val() == '') {
		alert('请输入验证码');
		$('#checkyzm').focus();
	} else {
		$.ajax({
			url : "<?php echo base_url();?>index/login/check",
			data : {
				dbg_uname : $('#username').val(),
				dbg_upwd : $('#password').val(),
				dbg_yzm : $('#checkyzm').val()
			},
			type : 'POST',
			async : false,
			success : function(result) {
...
// 判断邮箱地址是否正确
function is_Email(ajaxurl, prompt) {
	$.ajax({
		url : ajaxurl + "user/isemail",
		data : {
			isemail : $('#' + object).val()
		},
		type : 'POST',
...
function dbgjs_passport(type, ajaxurl) {
	switch (type) {
	/* 退出登录 */
	case 'quit':
		if (confirm("确认要退出系统？")) {
			$.ajax({
				url : ajaxurl + "user/passport/quit/",
				type : 'POST',
				async : false,
				success : function(result) {
					if (result == 1) {
						// location.reload();
						window.location.href = ajaxurl + 'user/passport/login';
					}
...
function translateBody(fobj) {
	...
	for (var i = 0; i < objs.length; i++) {
		var obj = objs.item(i);
		if (obj.tagName == "OBJECT" || obj.tagName == "SCRIPT" || obj.tagName == "EMBED")
			continue;
