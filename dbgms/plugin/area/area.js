...
function getCity(province){  //获取城市
    var cityNames = [];
    var cities, i, j ;
...
function getArea(province,city){  //获取县区
    var areaNames = [];
    var cities, i, j;
    for(i=0;i<cityStr.length;i++){
        if(province === cityStr[i].name){
...
function _init_area(){  //初始化
    $(".s_province").empty();
    $(".s_city").empty();
    $(".s_country").empty();

	var province_arr = getProvince();  //省份
    var province_html = '';
    for(var i=0;i<province_arr.length;i++){
        province_html = province_html+"<option name=''  value='"+province_arr[i]+"'>"+province_arr[i]+"</option>";
    }
    $(".s_province").append(province_html);

    var city_arr = getCity(province_arr[0]);  //城市
    var city_html = '';
    for(var i=0;i<city_arr.length;i++){
        city_html = city_html+"<option name=''  value='"+city_arr[i]+"'>"+city_arr[i]+"</option>";
    }
    $(".s_city").append(city_html);

    var county_arr = getArea(province_arr[0],city_arr[0]);  //县区
    var county_html = '';
    for(var i=0;i<county_arr.length;i++){
        county_html = county_html+"<option name=''  value='"+county_arr[i]+"'>"+county_arr[i]+"</option>";
    }
    $(".s_country").append(county_html);
}

$('.s_province').change(function () {   //改变省份修改城市和县区
    var num = $('.s_province').index(this); //判断点击的是第几个class
	var province_arr = document.getElementsByClassName('s_province');  //获取所有class s_province的标签
    var city_html = '';
    var province_name = province_arr[num].value; //取onchange标签的值

    $(".s_province").val(province_name); //给select赋值

    var city_arr = getCity(province_name);
    for(var i=0;i<city_arr.length;i++){
        city_html = city_html+"<option name=''  value='"+city_arr[i]+"'>"+city_arr[i]+"</option>";
    }
    ...
    // var new_arr =$(".s_city");
    // new_arr.splice(0,new_arr.length);
    // new_arr.push($(".s_city")[num]);
    // new_arr.empty();
    // new_arr.append(city_html);
    // // console.log($(".s_city"))
    // console.log(num)


    if($('.s_country').length != 0){  //修改县区
        var county_arr = getArea(province_name,city_arr[0]);  //县区
        var county_html = '';
        for(var i=0;i<county_arr.length;i++){
            county_html = county_html+"<option name=''  value='"+county_arr[i]+"'>"+county_arr[i]+"</option>";
        }
        ...
        // var new_arr2 =$(".s_country");
        // new_arr2.splice(0,new_arr2.length);
        // new_arr2.push($(".s_country")[num]);
        // new_arr2.empty();
        // new_arr2.append(county_html);
	}

})

$('.s_city').change(function () {   //改变城市修改县区
    if($('.s_country').length != 0){
        var num = $('.s_city').index(this); //判断点击的是第几个class
        var province_arr = document.getElementsByClassName('s_province');  //获取所有class s_province的标签
        var city_arr = document.getElementsByClassName('s_city');  //获取所有class s_province的标签
        var area_arr_html = '';
        var city_name = city_arr[num].value; //取onchange标签的值
        var province_name = province_arr[num].value; //取onchange标签的值
        var area_arr = getArea(province_name,city_name);
        for(var i=0;i<area_arr.length;i++){
            area_arr_html = area_arr_html+"<option name=''  value='"+area_arr[i]+"'>"+area_arr[i]+"</option>";
        }

        var new_arr =$(".s_country");
        new_arr.splice(0,new_arr.length);
        new_arr.push($(".s_country")[num]);
        new_arr.empty();
        new_arr.append(area_arr_html);
	}
