...
(function(win,doc){
	...
	config = {
		...
		base:'',
		/*
		 * 模块依赖 { moduleName: { path: 'URL', type:'js|css',
	...
	publicData = {},
	// 公共数据回调堆栈
	publicDataStack = {}, isArray = function(e){
		...
		var wait, n, t, img, done = function(){
			loaded[url] = 1;
			cb && cb(url);
			cb = null;
		...
		wait = win.setTimeout(function(){
			 ...
			 * 延时回调的意义是log延时长的URI，这个处理不属于加载器本身的功能移到外部 没有跳过是为了避免错误。
			 */
	...
	loadDeps = function(deps,cb){
		var mods = config.mods, id, m, mod, i = 0, len;
		id = deps.join('');
		len = deps.length;
		if(loadList[id]){
			cb();
