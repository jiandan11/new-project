...
<div class="dbg_warp">
 <form method="post" id="DbgMsFormEdit" name="DbgMsFormEdit" enctype="multipart/form-data">
  <input type="hidden" readonly="readonly" value="<?php echo ['id'];?>" name="id" /> <input type="hidden" readonly="readonly" value="update" name="act" /><input type="hidden" readonly="readonly" value="modelid" name="<?php echo $model['id'];?>" />
  <!-- 常规信息 -->
   ...
   <table class="subtab">
    <tbody>
     <tr>
      ...
      <td>
       <div class="hpft">
        <input type="text" class="itxt biaoci" name="title" id="title" style="width: 474px" value="<?php echo ['title'];?>"> <a href="###" id="signBtn">检测唯一性</a>
       </div>
      </td>
     </tr>
     <tr>
      ...
      <td>
       <div class="hpft">
        <input type="text" class="itxt" name="intime" style="width: 180px" value="<?php if(empty(['intime'])){echo date("Y-m-d H:i:s",time());}else{echo date("Y-m-d H:i:s", ['intime']);}?>" /> &nbsp;&nbsp;&nbsp; 最后更新时间：
       </div> <input type="text" class="itxt" name="uptime" style="width: 180px" title="留空则以当前真实时间为准" value="<?php if(empty(['uptime'])){echo date("Y-m-d H:i:s",time());}else{echo date("Y-m-d H:i:s", ['uptime']);}?>">
      </td>
     </tr>
     <tr>
      <td class="ft">责任编辑：</td>
      <td>
       <div class="hpft">
        <input type="hidden" readonly="readonly" value="<?php echo $dbg_user['id'];?>" name="adminid"> <input type="text" class="itxt" name="adminname" style="width: 180px" value="<?php echo $dbg_user['name'];?>"> &nbsp;&nbsp;&nbsp; 文档状态： <select name="state" style="width: 150px">
        ...
        <script type="text/javascript">$(function(){$("select[name='state']").attr("value","<?php echo empty(['state'])?0:['state'];?>");});</script>
       </div>
      </td>
     </tr>
     <tr>
      <td class="ft">作者(来源)：</td>
      <td><input type="hidden" readonly="readonly" value="<?php echo empty(['id'])?0:['authorid'];?>" name="authorid"> <input type="text" readonly="readonly" name="author" class="itxt" style="width: 180px; color: #999;" value="<?php if(!empty(['username'])){echo ['username'];}else{echo $dbg_user['alias'];};?>"> <input type="text" style="width: 180px;" class="itxt" value="<?php echo DBG_TITLE;?>"
       onfocus="if (value =='<?php echo DBG_TITLE;?>'){value='';this.style.color='000000'}" onblur="if (value ==''){value='<?php echo DBG_TITLE;?>';this.style.color='999999'}"></td>
     </tr>
     <tr>
      <td class="ft">推荐位置：</td>
      <td>
...
<?php dbg_diyfield ( 'load', 'column', $model,['columnid'] );?>
<?php foreach($diyfields as $k=>$val){$val['path'] = $model['sign'];dbg_diyfield ( 'load', $val['type'], $val, [$val['field']] );}?>
      <tr>
      <td class="ft">关键词：</td>
      <td><input type="text" name="keywords" id="keywords" class="itxt" value="<?php echo ['keywords'];?>" /> <span id="get_keywords" class="u-ico f-csp"><span class="icon"></span> 提取</span></td>
     </tr>
     <tr>
      <td class="ft">描述：</td>
      <td>
       <div class="hpft">
        <textarea name="description" id="description" class="diyfield_description"><?php echo ['description'];?></textarea>
 ...
 <div class="dbg_top">
  ...
  <div class="span_r">
   <form name="topSearchform" id="topSearchform" action="<?php echo $curr_url;?>" method="get">
    <!--  -->
    <input type="hidden" name="model" value="<?php echo $search['model'];?>" /> <input type="hidden" name="con" value="content" />
    <!--  -->
