...
<div class="dbgms_tabs_wrap">
 ...
 <ul id="dbgms_tabs">
  <li><a class="on" href="javascript:void(0);" onclick="dbgjs_tab(this,'tab1')"><span class="icon icon-info-sign"></span> 常规信息</a></li>
  <li><a href="javascript:void(0);" onclick="dbgjs_tab(this,'tab2')"><span class="icon icon-cog"></span> 高级参数</a></li>
 </ul>
 <div class="more">
  ...
  <div id="tab1" style="display: block;" class="dbgms_tab">
   <table class="subtab">
    <tbody>
     ...
     <tr class="invisible">
      <td class="ft">英文文档标题：</td>
      <td>
       <div class="hpft">
        <input type="text" class="itxt biaoci" name="etitle" id="title" style="width: 474px" value="<?php echo $row['etitle'];?>"> <a href="###" id="signBtn">检测唯一性</a>
       </div>
      </td>
     </tr>
     <tr>
     ...
     <tr>
      ...
      <td>
       <div class="hpft">
        <!--  -->
        <input type="hidden" readonly="readonly" value="<?php echo $_admin['id'];?>" name="adminid">
        <!--  -->
        <input type="text" class="itxt" style="width: 180px" value="<?php echo $_admin['name'];?>"> &nbsp;&nbsp;&nbsp;
        <!--  -->
        文档状态： <select name="state" style="width: 150px">
     ...
     <tr>
      ...
      <td>
       <div>
        <input type="text" value="<?php echo empty($row['authorid'])?'':$row['authorid'];?>" name="authorid">
       </div>
      </td>
     </tr>
     <tr>
...
<?php dbg_diyfield ( 'load', 'column', $model,$row);?>
<?php foreach($diyfields as $k=>$val){$val['path'] = $model['sign'];dbg_diyfield ( 'load', $val['type'], $val, $row[$val['field']] );}?>
      <tr>
      <td class="ft">描述：</td>
      <td>
       <div class="hpft">
        <textarea name="description" id="description" class="cms-content-description"><?php echo $row['description'];?></textarea>
        <span class="cms-content-tiqu-description"><a href="javascript:;" id="get_description"><span class="icon-signin"></span> 提取描述</a></span>
       </div>
      </td>
     </tr>
     <tr>
      <td class="ft">关键词：</td>
      <td><input type="text" name="keywords" id="keywords" class="itxt" value="<?php echo $row['keywords'];?>" /><span id="get_keywords" class="cms-content-tiqu-keyword"><span class="icon-signin"></span> 提取</span></td>
     </tr>
    </tbody>
   </table>
  </div>
 ...
 <script type="text/javascript">
  ...
  $(document).ready(function() { 
   ...
   $('#get_description').click(function() {
          var content= UE.getEditor('editorcontent').getContentTxt();
...
<option <?php if($val['property']==2){echo ' disabled="disabled"" ';}?> style="font-weight: bold;" value="<?php echo $val['id'];?>"><?php echo $val['name'];?></option>
<?php if (!empty($val['list'])){foreach( $val['list'] as $val2):?>
<option <?php if($val2['property']==2){echo ' disabled="disabled"" ';}?> style="font-weight: bold;" value="<?php echo $val2['id'];?>">&nbsp;&nbsp;|-->&nbsp;<?php echo $val2['name'];?></option>
<?php if (!empty($val2['list'])){foreach( $val2['list'] as $val3):?>
<option <?php if($val3['property']==2){echo ' disabled="disabled"" ';}?> style="font-weight: bold;" value="<?php echo $val3['id'];?>">&nbsp;&nbsp;&nbsp;&nbsp;|---▶ &nbsp;<?php echo $val3['name'];?></option>
<?php endforeach;}?>
<?php endforeach;}?>
<?php endforeach;?>
    ...
    </select>&nbsp;&nbsp;&nbsp;<input type="button" value="搜索 " class="dbgms_btn" onclick="cmsContentSearch()">
    <!--  -->
    <input type="button" value="更新所有缓存" class="dbgms_btn" onclick="cmsContentUpcache('all');">
   </form>
  </div>
 </div>
 <table class="tblist">
  <thead>
   <tr>
    <th width="3%">选</th>
    <th width="4%"><a href="javascript:void(0);" onclick="cmsContentOrder('id')">ID</a></th>
    <th width="6%">推荐位置</th>
    <th width="35%"><a href="javascript:void(0);" onclick="cmsContentOrder('title')">标题</a></th>
    <th><a href="javascript:void(0);" onclick="cmsContentOrder('columnid')">栏目</a></th>
    <th>状态(位置)</th>
    <th>点击量</th>
    <th>发布者</th>
    <th><a href="javascript:void(0);" onclick="cmsContentOrder('uptime')">更新时间</a></th>
    <th>审核员</th>
    <th>操作</th>
   </tr>
  </thead>
...
<?php endforeach;?>
    ...
    <td colspan="12"><div>
      <input onclick="cmsContentUpcache();" type="button" value="更新缓存" class="dbgms_btn"> <input type="button" onclick="dbgmsPause();" value="排序" class="dbgms_btn">
      <!--  -->
      <input type="button" onclick="dbgmsPause('该功能为定制功能~如有需要请联系开发者');" value="CDN" class="dbgms_btn"> <input type="button" onclick="dbgmsPause('该功能为定制功能~如有需要请联系开发者');" value="更新收录" class="dbgms_btn">
     </div></td>
   </tr>
  </tbody>
  <tfoot>
 ...
 <script type="text/javascript">
var DbgMsBaseUrl = '<?php echo $_dbgms_baseurl;?>';
var DbgMsSiteUrl = '<?php echo $_dbgms_url;?>';
var DbgMsModUrl = '<?php echo $mod_url;?>';
var DbgMsConUrl = '<?php echo $con_url;?>';
var DbgMsActUrl = '<?php echo $act_url;?>';
function cmsContentUpcache(type){
	if(type=='all'){
		location.href = DbgMsConUrl+'&act=upcache&ids='+ids;
	}else{
		var ids=dbgmsCheckGet();
		if(ids==''){
			alert('请至少选择一个');
			return ;
		}
