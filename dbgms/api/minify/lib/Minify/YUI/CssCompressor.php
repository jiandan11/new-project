...
class Minify_YUI_CssCompressor {
	...
	public function compress($css, $linebreakpos = 0)
	{
		...
		$css = preg_replace_callback ( "@rgb\\s*\\(\\s*([0-9,\\s]+)\\s*\\)@", array(
				$this,
				'_shortenRgbCB' 
		), $css );
		...
		$css = preg_replace_callback ( "@([^\"'=\\s])(\\s*)#([0-9a-fA-F])([0-9a-fA-F])([0-9a-fA-F])([0-9a-fA-F])([0-9a-fA-F])([0-9a-fA-F])@", array(
				$this,
				'_shortenHexCB' 
		), $css );
	...
	protected function _shortenHexCB($m)
	{
		...
		if((strtolower ( $m[3] ) === strtolower ( $m[4] )) && (strtolower ( $m[5] ) === strtolower ( $m[6] )) && (strtolower ( $m[7] ) === strtolower ( $m[8] )))
		{
