...
class Minify_JS_ClosureCompiler {
	...
	{
		$this->_fallbackFunc = isset ( $options['fallbackMinifier'] ) ? $options['fallbackMinifier'] : array(
				$this,
				'_fallback' 
		);
	}
	public function min($js)
	{
		...
		if($bytes > 200000)
		{
			throw new Minify_JS_ClosureCompiler_Exception ( 'POST content larger than 200000 bytes' );
		}
	...
	protected function _getResponse($postBody)
	{
		...
		{
			throw new Minify_JS_ClosureCompiler_Exception ( "Could not make HTTP request: allow_url_open is false and cURL not available" );
		}
		if(false === $contents)
		{
