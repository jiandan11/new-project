...
abstract class Minify_Controller_Base {
	...
	public function getDefaultMinifers()
	{
		...
		$ret[Minify::TYPE_CSS] = array(
				'Minify_CSS',
				'minify' 
		);
		$ret[Minify::TYPE_HTML] = array(
				'Minify_HTML',
				'minify' 
		);
		return $ret;
	...
	public static function _fileIsSafe($file, $safeDirs)
	{
		...
		return in_array ( strrev ( $revExt ), array(
				'js',
				'css',
				'html',
				'txt' 
	...
	public static function checkAllowDirs($file, $allowDirs, $uri)
	{
		...
		throw new Exception ( "File '$file' is outside \$allowDirs. If the path is" . " resolved via an alias/symlink, look into the \$min_symlinks option." . " E.g. \$min_symlinks['/" . dirname ( $uri ) . "'] = '" . dirname ( $file ) . "';" );
	}
